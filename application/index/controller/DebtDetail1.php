...
class DebtDetail1 extends Common
{
    ...
    public function save()
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            $map['payableProfit'] = input('request.payableProfit/a')[$i];//应交利润
            $map['longdebt'] = input('request.longdebt/a')[$i];//一年内到期的长期负债
            $map['payableDebt'] = input('request.payableDebt/a')[$i];
            $map['longPayableDebt'] = input('request.longPayableDebt/a')[$i];
            $map['otherLongDebt'] = input('request.otherLongDebt/a')[$i];
            $map['houseTurnover'] = input('request.houseTurnover/a')[$i];
            $map['specialPayable'] = input('request.specialPayable/a')[$i];
            $map['dydksx'] = input('request.dydksx/a')[$i];
            $map['shareholderEquity'] = input('request.shareholderEquity/a')[$i];
            $map['other'] = input('request.other/a')[$i];
