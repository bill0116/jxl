 ...
 * Func: LC限额申请
 * FlowID: 22
 * TableName: Z_TB_FlowApp_LcQuotaApply
 */
...
class Lc extends Common
{
    ...
    public function update()
    {
        if (request()->isPost()) {
            $map = input('post.');
            $db = Db::table('Z_TB_FlowApp_LcQuotaApply');
            if ($map['tasktype'] == 0) {
                $param = request()->except(['ID', 'activeid', 'tasktype', 'routeid', 'userid', 'IsApproved'], 'post');
                $param['trafficCode'] = empty($map['trafficCode']) ?: implode(',', $map['trafficCode']);
    ...
    public function index()
    {
        if (request()->isPost()) {
            ...
            $id = Db::table('Z_TB_FlowApp_LcQuotaApply')->insertGetId($param);
            if ($id) {
                $activeid = $this->model->newStartActive(22, $id, $param['Creator']);
                if ($activeid) {
                    $myActive = $this->model->getFlowMyactive($activeid, $param['Creator']);
                    $this->assign('myActive', $myActive);
                    $resultData = Db::table('Z_TB_FlowApp_LcQuotaApply')->where('id=' . $id)->find();
                    $this->assign('data', $resultData);
            ...
            $myActive = $this->model->getFlowMyactive($map['activeid'], session('auth_id'));
            $this->assign('myActive', $myActive);
            $resultData = Db::table('Z_TB_FlowApp_LcQuotaApply')->where('id', $map['id'])->find();
            $this->assign('data', $resultData);
            $list = $this->model->getUserApprove($map['activeid']);
            $this->assign('approveList', $list);
            $attachList = $this->getAttachFile($resultData['Attaches']);
